...
let g:colors_name='gruvbox_community'
...
if !exists('g:gruvbox_community_bold')
  let g:gruvbox_community_bold=1
endif
if !exists('g:gruvbox_community_italic')
  if has('gui_running') || $TERM_ITALICS == 'true'
    let g:gruvbox_community_italic=1
  else
    let g:gruvbox_community_italic=0
  endif
endif
if !exists('g:gruvbox_community_undercurl')
  let g:gruvbox_community_undercurl=1
endif
if !exists('g:gruvbox_community_underline')
  let g:gruvbox_community_underline=1
endif
if !exists('g:gruvbox_community_inverse')
  let g:gruvbox_community_inverse=1
endif

if !exists('g:gruvbox_community_guisp_fallback') || index(['fg', 'bg'], g:gruvbox_community_guisp_fallback) == -1
  let g:gruvbox_community_guisp_fallback='NONE'
endif

if !exists('g:gruvbox_community_improved_strings')
  let g:gruvbox_community_improved_strings=0
endif

if !exists('g:gruvbox_community_improved_warnings')
  let g:gruvbox_community_improved_warnings=0
endif

if !exists('g:gruvbox_community_termcolors')
  let g:gruvbox_community_termcolors=256
endif

if !exists('g:gruvbox_community_invert_indent_guides')
  let g:gruvbox_community_invert_indent_guides=0
endif

if exists('g:gruvbox_community_contrast')
  echo 'g:gruvbox_community_contrast is deprecated; use g:gruvbox_community_contrast_light and g:gruvbox_community_contrast_dark instead'
endif

if !exists('g:gruvbox_community_contrast_dark')
  let g:gruvbox_community_contrast_dark='medium'
endif

if !exists('g:gruvbox_community_contrast_light')
  let g:gruvbox_community_contrast_light='medium'
endif
...
function! s:Color(name, default, ...)
  ...
  if has_key(s:gb, a:name)
    ...
    elseif type(l:color) == type([])
      ...
      " invalid value
      echo a:name 'is invalid, usage: let g:gruvbox_community_colors.color = (["#ffffff", 255]|"#ffffff"|255)'
      return 0
    endif
...
" get the global gruvbox_community palette options, if any
let g:gruvbox_community_colors = get(g:, 'gruvbox_community_colors', {})
" initialize the script palette
let s:gb = copy(g:gruvbox_community_colors)
let g:current_gruvbox_community_colors = s:gb
...
let s:bold = 'bold,'
if g:gruvbox_community_bold == 0
  let s:bold = ''
endif

let s:italic = 'italic,'
if g:gruvbox_community_italic == 0
  let s:italic = ''
endif

let s:underline = 'underline,'
if g:gruvbox_community_underline == 0
  let s:underline = ''
endif

let s:undercurl = 'undercurl,'
if g:gruvbox_community_undercurl == 0
  let s:undercurl = ''
endif

let s:inverse = 'inverse,'
if g:gruvbox_community_inverse == 0
  let s:inverse = ''
endif
...
if s:is_dark
  ...
  if g:gruvbox_community_contrast_dark == 'soft'
    let s:bg0  = s:gb.dark0_soft
  elseif g:gruvbox_community_contrast_dark == 'hard'
  ...
  if g:gruvbox_community_contrast_light == 'soft'
    let s:bg0  = s:gb.light0_soft
  elseif g:gruvbox_community_contrast_light == 'hard'
...
if g:gruvbox_community_termcolors == 16
  let s:bg0[1]    = 0
  let s:fg4[1]    = 7
  let s:gray[1]   = 8
  let s:red[1]    = 9
...
if exists('g:gruvbox_community_hls_cursor')
  let s:hls_cursor = get(s:gb, g:gruvbox_community_hls_cursor)
endif

let s:hls_highlight = s:gb.yellow
if exists('g:gruvbox_community_hls_highlight')
  let s:hls_highlight = get(s:gb, g:gruvbox_community_hls_highlight)
endif

let s:number_column = s:none
if exists('g:gruvbox_community_number_column')
  let s:number_column = get(s:gb, g:gruvbox_community_number_column)
endif

let s:sign_column = s:gb.bg1

if exists('g:gruvbox_community_sign_column')
  let s:sign_column = get(s:gb, g:gruvbox_community_sign_column)
endif

let s:color_column = s:gb.bg1
if exists('g:gruvbox_community_color_column')
  let s:color_column = get(s:gb, g:gruvbox_community_color_column)
endif

let s:vert_split = s:gb.bg0
if exists('g:gruvbox_community_vert_split')
  let s:vert_split = get(s:gb, g:gruvbox_community_vert_split)
endif

let s:invert_signs = ''
if exists('g:gruvbox_community_invert_signs')
  if g:gruvbox_community_invert_signs == 1
    let s:invert_signs = s:inverse
  endif
endif

let s:invert_selection = s:inverse
if exists('g:gruvbox_community_invert_selection')
  if g:gruvbox_community_invert_selection == 0
    let s:invert_selection = ''
  endif
endif

let s:invert_tabline = ''
if exists('g:gruvbox_community_invert_tabline')
  if g:gruvbox_community_invert_tabline == 1
    let s:invert_tabline = s:inverse
  endif
endif

let s:tabline_sel = s:gb.green
if exists('g:gruvbox_community_tabline_sel')
  let s:tabline_sel = get(s:gb, g:gruvbox_community_tabline_sel)
endif

let s:italicize_comments = s:italic
if exists('g:gruvbox_community_italicize_comments')
  if g:gruvbox_community_italicize_comments == 0
    let s:italicize_comments = ''
  endif
endif

let s:italicize_strings = ''
if exists('g:gruvbox_community_italicize_strings')
  if g:gruvbox_community_italicize_strings == 1
...
function! s:HL(group, fg, ...)
  ...
  " special fallback
  if a:0 >= 3
    if g:gruvbox_community_guisp_fallback != 'NONE'
      let fg = a:3
    endif

    " bg fallback mode should invert higlighting
    if g:gruvbox_community_guisp_fallback == 'bg'
      let emstr .= 'inverse,'
    endif
  endif
...
" --- https://github.com/morhetz/gruvbox_community/issues/7
if exists('v:vim_did_enter')
  let g:gruvbox_community_vim_did_enter = v:vim_did_enter
else
  augroup GruvboxVimEnter
    au!
    autocmd VimEnter * let g:gruvbox_community_vim_did_enter = 1
  augroup End
endif
if get(g:, 'gruvbox_community_vim_did_enter', 0)
  if s:is_dark
    set background=dark
  else
...
if g:gruvbox_community_improved_strings == 0
...
if has("spell")
  ...
  if g:gruvbox_community_improved_warnings == 0
...
if g:indent_guides_auto_colors == 0
  if g:gruvbox_community_invert_indent_guides == 0
