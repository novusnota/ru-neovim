...
" 'filename', вместо него - absolutepath
let s:_lightline = {
      \   'active': {
      \     'left': [['mode', 'paste'], ['readonly', 'absolutepath', 'modified'], ['statuslinetabs']],
      ...
      \     'lineinfo': '%3l:%-2v', 'line': '%l', 'column': '%c%V', 'close': '%999X X ', 'winnr': '%{winnr()}'
      \   },
...
function! LightlineStatuslineTabs() abort
  return join(map(range(1, tabpagenr('$')),
        \ '(v:val == tabpagenr() ? "" : "[".v:val.": ".lightline#tab#filename(v:val)."] ")'), "")
endfunction

" lineinfo from '%3l:%-2c'
" Was like 'space thing space'
" thing: \u2b81 
