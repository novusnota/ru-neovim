...
" AUTHOR: zsugabubus
" https://github.com/zsugabubus/vim-jumpmotion
...
if !hasmapto('<Plug>(JumpMotion)')
  for s:lhs in ['s', '<C-a>', '<Space>', '<Leader><Leader>', '<Leader><Space>', '<Leader>s']
...
if !exists('*JumpMotionKey')
  ...
  function JumpMotionKey(nth) abort
    let alphabet = get(g:, 'jumpmotion_abc', 'abcdefghijklmnopqrstuvwxyz123456789')
    let nth = a:nth
    let str = ''
